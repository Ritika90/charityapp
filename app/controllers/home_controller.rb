class HomeController < ApplicationController
  ...
  def home_page_feed
    @tz = Geocoder.search(current_user.current_sign_in_ip)[0].data["timezone"]
 	  @charityusers = Bet.where(:active=>true)
  end	
  ...
  def manage_purchase
    ...
    @transaction= PaymentDetail.where(:receiver_email => @user_email).group('bet_id').paginate(:page => params[:page], :per_page => 6)
    else
    @user_email=current_user.paypal_email
    @transaction= PaymentDetail.where(:sender_email => @user_email).group('bet_id').paginate(:page => params[:page], :per_page => 6)
